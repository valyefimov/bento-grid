<script lang="ts">
  ...
  let selfLevel = $derived(
    level.controls.includes('justifyself') || level.controls.includes('alignself')
  );
  ...
  let gridStyle = $derived.by(() => {
    ...
    parts.push(
      `grid-auto-rows: ${game.values.autoRows === 'auto' ? 'auto' : game.values.autoRows + 'px'}`
    );
  ...
  let heroStyle = $derived.by(() => {
    ...
    if (areasMode) {
      ...
      if (subgridLevel) {
        ...
        parts.push(
          `grid-template-columns: ${game.values.subgridMode === 'subgrid' ? 'subgrid' : 'repeat(2, 1fr)'}`
        );
      } else {
  ...
  function panelStyle(id: string): string {
    ...
    if (orderLevel && id === (level.orderTargetId ?? 'p2'))
      parts.push(`order: ${game.values.order}`);
    return parts.join('; ');
  }
...
<div class="comic-page-wrap">
  <div class="overlay-toggles">
    <button
      class="toggle-btn"
      class:active={ui.showGridLines}
      onclick={() => (ui.showGridLines = !ui.showGridLines)}
    >
      {t.ui.toggleLines}
    </button>
    <button class="toggle-btn" class:active={ui.negLine1Active} onclick={toggleNeg1}
      >{t.ui.toggleNeg1}</button
    >
    <button class="toggle-btn" class:active={ui.negLine2Active} onclick={toggleNeg2}
      >{t.ui.toggleNeg2}</button
    >
  </div>
  ...
  {#key ui.shakeKey}
    <div
      class="comic-grid"
      class:show-lines={ui.showGridLines}
      class:shake={ui.shakeKey > 0}
      style={gridStyle}
    >
      {#each PANEL_IDS as id, i (id)}
        {#if id === 'heroPanel'}
          <div
            class="panel-action {PANEL_CLASSES[i]}"
            class:hidden-panel={isHidden(id)}
            style={heroStyle}
          >
            {#if subgridLevel}
  ...
  {#if ui.feedbackKind}
    <div
      class="feedback-msg show"
      class:ok={ui.feedbackKind === 'ok'}
      class:err={ui.feedbackKind === 'err'}
    >
      {ui.feedbackText}
    </div>
  {/if}
